...
///Meme principe que actualiser boss///
...
void actualiserBoss3(t_perso *boss,t_liste* a)
{
    ...
    {
        if(boss->cmpt<150)      ///Le boss se déplace de maniere homogene tous les 100 pixels
        {
            if(boss->x>3300)        ///On determine des limites que le boss ne doit pas dépasser
            {
                boss->dx=-1;
            }
            if(boss->x<2500)
